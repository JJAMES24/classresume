...
test('Check Experience Section', async ({ page }) => {
  const h3Elements = await page.$$('.experience-section h3');
...
test('Check Education Degree and University', async ({ page }) => {
  ...
  const h3Elements = await page.$$('.education-section h3');
  for (const h3Element of h3Elements) {
    const isVisible = await h3Element.isVisible();
    expect(isVisible).toBe(true);
  }
...
test('Check Education Duration', async ({ page }) => {
  ...
  const h4Elements = await page.$$('.education-section h4');
  for (const h4Element of h4Elements) {
    const isVisible = await h4Element.isVisible();
    expect(isVisible).toBe(true);
  }
});
